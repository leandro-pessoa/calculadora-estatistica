...
const Results = () => {
    ...
    return (
        <Container>
            ...
            {
                uniqueResults.rol || compostResults.rolX && compostResults.rolY ?
                <>
                    ...
                    <StyledDiv>
                        {
                            uniqueResults.numbers.length >= 1 && uniqueResults.rol &&
                                    ...
                                    <StyledCalcsUl>
                                        {
                                            uniqueResults.numbers.map(calc => {
                                                if(calc.moda){
                                                    return (
                                                        <li key={uuidv4()}>
                                                            <span>{calc.calcName}:</span>
                                                            {
                                                                typeof calc.moda === "string" ?
                                                                    <>inexistente</>
                                                                :
                                                                    calc.moda.map(num =>
                                                                        <p>{num}</p>
                                                ...
                                                return (
                                                    <li key={uuidv4()}>
                                                        <span>{calc.calcName}:</span> 
                                                        {calc.number?.toFixed(Number(decimalPlaces))}
                                                    </li>
                                                )
                        ...
                        <StyledDiv>
                        {
                            compostResults.numbers.length >= 1 && compostResults.rolX && compostResults.rolY &&
                                    ...
                                    <StyledCalcsUl>
                                        {
                                            compostResults.numbers.map(calc => {
                                                if(calc.regressao) {
                                                    const regressaoSimbol = calc.regressao[1] >= 0 ? <PiPlus/> : <PiMinus/>

                                                    return <li key={uuidv4()}>
                                                        <span>{calc.calcName}:</span>
                                                        y <PiEquals/>
                                                         {calc.regressao[0].toFixed(Number(decimalPlaces))}
                                                         {regressaoSimbol}
                                                         {Math.abs(calc.regressao[1]).toFixed(Number(decimalPlaces))}x
                                                    </li>
                                                }
                                                return <li key={uuidv4()}>
                                                    <span>{calc.calcName}:</span> {calc.number?.toFixed(Number(decimalPlaces))}
                                                </li> 
                                            })
                                        }
                                    </StyledCalcsUl>
