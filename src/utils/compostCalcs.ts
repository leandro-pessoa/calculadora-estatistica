...
const covariancia = (x: number[], y: number[]) => {
    ...
    const produtoSomatorio = produtos(somatorioX, somatorioY).reduce(
        (acc, value) => value + acc,
    )
...
const correlacao = (x: number[], y: number[]) => {
    ...
    const numerador =
        x.length * somatorio(produtosXY) - somatorio(x) * somatorio(y)

    // cálculos no denominador
    const xQuadrados = x.map((num) => num ** 2)
    const yQuadrados = y.map((num) => num ** 2)
    const calcsX = Math.sqrt(
        x.length * somatorio(xQuadrados) - Math.pow(somatorio(x), 2),
    )
    const calcsY = Math.sqrt(
        y.length * somatorio(yQuadrados) - Math.pow(somatorio(y), 2),
    )
...
export { covariancia, correlacao, regressaoLinear }
